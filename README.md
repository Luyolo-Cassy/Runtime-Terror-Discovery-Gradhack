...
- **`main.py`** (repo root) — a standalone Flask microservice covering just
  the Profile responsibility (`GET /profile/{user_id}`,
  `GET /profile/{user_id}/evolution`), built independently against the raw
  `dataset` table with its own override logic. It's not called by the
  frontend — `backend/main.py`'s `/api/profile` and `/api/profile/{id}/evolution`
  are what the app actually uses. Kept here as a self-contained, deployable
  reference for the Profile role, and as a working cross-check: both
  implementations independently compute the same healthy-spend ratio for the
  same customer (see the "Assumptions to verify" section below for the one
  thing they're each doing slightly differently — retailer vs. category).
