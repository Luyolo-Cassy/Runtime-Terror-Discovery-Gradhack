...
def _autoseed_pantry(user_id: str, baskets: int = 2):
    """
    Seed an empty pantry from the customer's most recent partner baskets.

    `user_pantry` is a table this API writes to - it only fills when someone
    scans a slip or imports a basket. A customer we have never interacted with
    therefore shows an empty kitchen even though BigQuery holds their entire
    purchase history, which is the opposite of requirement 4.2 ("track what
    they bought in the past week/month").

    So on the first read for a user with nothing stored, we pull their latest
    baskets through the same import path the UI uses. This runs once: after
    seeding, list_pantry is non-empty and this is skipped on every later call.
    Only HealthyFood lines are stored, matching import_basket's behaviour.
    """
    receipts = bq.safe(
        lambda: receipts_service.list_receipts(user_id, limit_baskets=baskets),
        [], "autoseed_receipts",
    )
    if not receipts:
        log.info("autoseed_pantry: no baskets in BigQuery for user_id=%s", user_id)
        return []

    seeded = []
    for r in receipts[:baskets]:
        basket_id = r.get("id")
        if not basket_id:
            continue
        items = bq.safe(
            lambda bid=basket_id: receipts_service.basket_items(user_id, bid, True),
            [], "autoseed_items",
        )
        if not items:
            continue
        rows, errors = pantry_service.add_items(user_id, items)
        if errors:
            log.warning("autoseed_pantry: insert failed for basket_id=%s: %s", basket_id, errors)
            continue
        seeded.extend(rows)

    log.info("autoseed_pantry: seeded %d item(s) for user_id=%s from %d basket(s)",
             len(seeded), user_id, len(receipts[:baskets]))
    return seeded
...
def get_pantry(user_id: str):
    ...
    if not items:
        # Never seen this customer in-app: build their kitchen from BigQuery.
        _autoseed_pantry(user_id)
        items = bq.safe(lambda: pantry_service.list_pantry(user_id), [], "list_pantry_reseed")
...
def get_home(user_id: str):
    ...
    # Same first-load seeding as GET /api/pantry, so switching customers shows a
    # populated kitchen straight away rather than an empty one.
    _pantry = bq.safe(lambda: pantry_service.list_pantry(user_id), [], "home_pantry")
    if not _pantry:
        _autoseed_pantry(user_id)
        _pantry = bq.safe(lambda: pantry_service.list_pantry(user_id), [], "home_pantry_reseed")
