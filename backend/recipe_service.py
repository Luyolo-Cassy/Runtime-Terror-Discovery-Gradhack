...
# PROMPT + PARSING
# ---------------------------------------------------------------------------
def build_prompt(pantry, profile, catalogue):
...
def generate_personalized_recipe(bq_client, model, dataset: str, user_id: str):
    """
    Returns a dict ready to serialise as the API response, plus a `_save_row`
    the caller can insert into saved_recipes. Raises nothing user-facing.
    """
    ...
    prompt = build_prompt(pantry, profile, catalogue)
